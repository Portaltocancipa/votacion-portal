...
import { actualizarParqueadero, borrarParqueadero, validarParqueadero } from "@/lib/parqueaderos";
...
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await actualizarParqueadero(id, body.correo, body, body.token);
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    await borrarParqueadero(id, body.correo, body.token);
