...
import { actualizarMascota, borrarMascota, validarMascota } from "@/lib/mascotas";

export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  const { id } = await props.params;
  const body = await req.json();
  const errorValidacion = validarMascota(body);
  if (errorValidacion) return NextResponse.json({ error: errorValidacion }, { status: 400 });

  try {
    const data = await actualizarMascota(id, body.correo, body, body.token);
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    await borrarMascota(id, body.correo, body.token);
