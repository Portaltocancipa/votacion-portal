...
import { crearMudanza, listarMudanzasPorCorreo, validarMudanza } from "@/lib/mudanzas";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await listarMudanzasPorCorreo(correo);
...
export async function POST(req: NextRequest) {
  ...
  const errorValidacion = validarMudanza(body);
  ...
  try {
    const data = await crearMudanza(body, body.token);
    return NextResponse.json(data);
  } catch (e) {
