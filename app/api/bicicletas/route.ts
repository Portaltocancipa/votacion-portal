...
import { crearBicicleta, listarBicicletasPorCorreo, validarBicicleta } from "@/lib/bicicletas";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await listarBicicletasPorCorreo(correo);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const data = await crearBicicleta(body, body.token);
    return NextResponse.json(data);
  } catch (e) {
